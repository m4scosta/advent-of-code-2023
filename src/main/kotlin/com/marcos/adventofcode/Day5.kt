...
internal class Day5 : Solution {
    ...
    override fun solve(input: InputStream) {
        ...
        val seeds = parseSeeds(inputIterator.next())
        ...
        val minLocationNumber = seeds
            .minOf { seedNumber ->
                categoryMapsPipeline.fold(seedNumber) { currentValue, categoryMap ->
                    getNextCategoryNumber(currentValue, categoryMap)
                }
            }
    ...
    private fun parseSeeds(seedsLine: String): List<Long> {
    ...
    private fun getNextCategoryNumber(sourceValue: Long, categoryMap: List<CategoryMapEntry>): Long {
        for (categoryMapEntry in categoryMap) {
            val destinationNumber = categoryMapEntry.getDestinationNumber(sourceValue)
    ...
    private data class CategoryMapEntry(
        ...
        fun getDestinationNumber(sourceValue: Long): Long? {
            if (sourceValue !in sourceRange) {
                return null
            }
