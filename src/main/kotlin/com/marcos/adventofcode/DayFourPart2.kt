...
internal class DayFourPart2 : Solution {
    ...
    override fun solve(input: InputStream) {
        ...
        val cards = input.bufferedReader().lines()
            .map { parseCardLine(it) }
            .toList()

        val numberOfCards = cards.map { 1L }.toMutableList()

        for ((index, card) in cards.withIndex()) {
            val matchingNumbersCount = card.getMatchingNumbersCount()

            for (nextCard in index + 1..index + matchingNumbersCount) {
                if (nextCard >= cards.size) {
                    break
                }

                numberOfCards[nextCard] += numberOfCards[index]
            }
        }
    ...
    private fun parseCardLine(cardLine: String): Card {
        val numberLists = cardLine
            .split(":")[1] // discard card identifier
            .split("|") // split winning numbers from numbers you have strings
            .map { Helpers.parseList(it, itemParser = String::toInt) }

        return Card(
            winningNumbers = numberLists.first().toSet(),
            numbersYouHave = numberLists[1].toSet(),
        )
    }

    private data class Card(
        private val winningNumbers: Set<Int>,
        private val numbersYouHave: Set<Int>,
    ) {
